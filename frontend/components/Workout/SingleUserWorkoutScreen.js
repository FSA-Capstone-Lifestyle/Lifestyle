...
import { fetchSingleUserWorkout } from "../../store/slices/singleUser.slice";
import {
  createExercise,
  fetchExercises,
  removeExercise,
...
const SingleWorkoutScreen = (props) => {
  ...
  const [isChecked, setIsChecked] = useState(exerciseState || null);
  ...
  const workoutComplete = () => {
    if (isChecked.every((exercise) => exercise.isCompleted)) {
      setProgress("Completed");
      setIsComplete(true);
    } else if (isChecked.some((exercise) => exercise.isCompleted)) {
  ...
  useEffect(() => {
    dispatch(
      fetchSingleUserWorkout({ userId: id, workoutId: props.route.params.id })
    );
  }, []);
  ...
  const handleStatusChange = (exercise) => {
    const newState = exerciseState.map((exerciseState) => {
      if (exerciseState === exercise) {
        exerciseState.isComplete = !exercise.isComplete;
      }
    });
                  ...
                  <Box key={exercise.id}>
                    ...
                    >
                      <Checkbox
                        aria-label="checkbox"
                        isChecked={false}
                        onChange={() => handleStatusChange(exercise)}
                        value={exercise.name}
                      />
                      <Text
                        width="100%"
